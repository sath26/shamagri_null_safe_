...
class SoldInvoice extends StatelessWidget {
  SoldInvoice({Key? key, this.soldId}) : super(key: key);
  final String? soldId;
  ...
  Widget build(BuildContext context) {
    _scrollController.addListener(() {
      ...
      if (maxScroll - currentScroll <= delta) {
        BlocProvider.of<ListSoldInvoiceBloc>(context)
            .add(ListSoldInvoiceEvent.watchAfterTen());
    ...
    return MultiBlocProvider(
      providers: [
        BlocProvider(
          create: (context) => getIt<ListSoldInvoiceBloc>()
            ..add(ListSoldInvoiceEvent.watchFirstTen(this.soldId)),
        ),
      ],
      child: MultiBlocListener(
          ...
          child: Builder(builder: (BuildContext context) {
            ...
            _scrollController.addListener(() {
              ...
              double delta = MediaQuery.of(context).size.height * 0.20;
              if (maxScroll - currentScroll <= delta) {
                BlocProvider.of<ListSoldInvoiceBloc>(context).state.map(
                    initial: (initial) => null,
                    loadInProgress: (loadInProgress) => null,
                    loadSuccess: (state) {
                      if (!(state.watchAfterTenCountIsZeroToNine!)) {
                        print('called loadsuccess');
                        return BlocProvider.of<ListSoldInvoiceBloc>(context)
                            .add(ListSoldInvoiceEvent.watchAfterTen());
                      }
                      return null;
                    },
