...
class TitleInputHook extends HookWidget {
  ...
  Widget build(BuildContext context) {
    final textEditingController = useTextEditingController(
        text: Get.put(TitleQuotationController()).titleQuotation);

    return BlocListener<QuotationBloc, QuotationState>(
        listenWhen: (p, c) => p.isEditing != c.isEditing,
        listener: (context, state) {
          textEditingController.text = state.quotation!.title!.getOrCrash();
        },
        child: Padding(
            ...
            child: TextFormField(
              ...
              minLines: 1,
              onChanged: (value) {
                Get.find<TitleQuotationController>().uxTitle(value);
                return context
                    .read<QuotationBloc>()
